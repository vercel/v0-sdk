...
export async function verifyChangesets(
  ...
  // Skip check if pull request has bypass label
  ...
  for (const path of env.CHANGED_FILES.trim().split(' ')) {
    ...
    if (invalidVersionBumps.length > 0) {
      throw Object.assign(
        new Error(
          `Invalid .changeset file - invalid version bump (only "patch" is allowed). To bypass, add one of the following labels: ${BYPASS_LABELS.join(', ')}`,
        ),
